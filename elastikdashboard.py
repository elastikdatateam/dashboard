...
if check_password():
    with st.container():
        ...
        with left_column:
            ...
            st.metric("Yesterdays Logins", df['users'].iloc[0], str(df['users'].iloc[0] - df['users'].iloc[1]))
        ...
        with right_column:
            ...
            # chart1 = alt.Chart(school_tests.T).mark_bar().encode(x='schoolName',y='assignedcomplete')
            # fig = plt.figure()
            # fig.facecolor('none')
            # sns.barplot(data=school_tests.T,x='schoolName',y='assignedcomplete')
            # st.pyplot(fig)

            st.bar_chart(school_tests.drop('schoolName', axis=0)[dict_school_keys[option]], use_container_width=True)
